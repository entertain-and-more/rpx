...
- **Local storage only:** Worlds, sessions, settings, rule sets, media paths, and backups are stored locally in `rpx_pro_data/`.
- **No account required:** RPX Pro does not require a cloud account or hosted backend.
- **AI prompts:** RPX Pro can generate prompts for external AI tools. The application copies or displays those prompts locally; it does not send them to an AI provider by itself.
...
- **Nur lokale Speicherung:** Welten, Sessions, Einstellungen, Regelwerke, Medienpfade und Backups werden lokal in `rpx_pro_data/` gespeichert.
- **Kein Account erforderlich:** RPX Pro benötigt kein Cloud-Konto und kein gehostetes Backend.
- **KI-Prompts:** RPX Pro kann Prompts für externe KI-Tools erzeugen. Die Anwendung kopiert oder zeigt diese Prompts lokal an; sie sendet sie nicht selbst an einen KI-Anbieter.
